// ================================================================
// T4i New Hire Data Sync Script — v8 Optimized
// Based on v8 with optimizations highlighted by: // >>> OPTIMIZED ... <<<
// ================================================================
...
// >>> OPTIMIZED: Add config for skip logging (optional debug) <<<
var SKIP_LOG_MAX = 10;
...
// Saves current destination sheet to hidden "_Undo" sheet. Call before any destructive operation.
...
// Restores destination sheet from last saved snapshot. Use after running sync/cleanup by mistake.
function recall() {
  var destSS = SpreadsheetApp.openById(DEST_SPREADSHEET_ID);
  var mainSheet = destSS.getSheets()[0];
  var undoSheet = destSS.getSheetByName('_Undo');
...
function sortAndFormatSheet_(destSheet) {
  ...
  var dataRows = lastRow - 1;  // numRows for data (row 2 to lastRow)
...
// Removes duplicate rows by composite key. Keeps first occurrence, drops later ones.
// Dedup key: FirstName|LastName|Username|StartDate|ShipDate|HireType|SourceTab (canonical).
// skipUndoSave: if true, do not save undo snapshot (used when called from fullSync/syncNewHireData).
function cleanupDuplicates(skipUndoSave) {
  if (!skipUndoSave) saveUndoSnapshot_();
  var destSS = SpreadsheetApp.openById(DEST_SPREADSHEET_ID);
  var destSheet = destSS.getSheets()[0];
  var lastRow = destSheet.getLastRow();
  if (lastRow <= 1) return;
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    var key = firstName + '|' + lastName + '|' + username + '|' + startStr + '|' + shipStr + '|' + hireType + '|' + sourceTab;

    if (seen[key]) {
      removed++;
      if (removedLog.length < REMOVED_LOG_MAX) {
        removedLog.push('[' + removed + '] ' + firstName + ' ' + lastName + ' | ' + username + ' | Start: ' + startStr + ' | Ship: ' + shipStr + ' | ' + sourceTab);
      }
      continue;
    }
    seen[key] = true;
...
function fullSync() {
  ...
  for (; s < sheets.length; s++) {
    ...
    if (rows.length > 0) {
      var lastRow = destSheet.getLastRow();
      // getRange(row, col, numRows, numCols) — 3rd param is numRows
  ...
  if (s >= sheets.length) {
    ...
    // >>> OPTIMIZED: Auto-run cleanupDuplicates at end of full sync <<<
    Logger.log('Cleaning up duplicates...');
    cleanupDuplicates(true);  // skip undo save (snapshot already taken at fullSync start)
...
function syncNewHireData() {
  ...
  // >>> OPTIMIZED: Auto-run cleanupDuplicates after incremental sync <<<
  cleanupDuplicates(true);  // skip undo save (snapshot already taken at syncNewHireData start)
...
// >>> OPTIMIZED: Reject more bad date formats — double slash (7//22/2025), dot separator (10/12.2021) <<<
function hasIrregularDate_(val) {
  ...
  if (str !== '' && str.indexOf('//') !== -1) return true;  // >>> OPTIMIZED: double slash <<<
  if (str !== '' && /\d[\/\-]\d{1,2}\.\d/.test(str)) return true;  // >>> OPTIMIZED: dot as separator <<<
  var d = new Date(val);
  if (!isNaN(d.getTime())) {
...
// >>> OPTIMIZED: NEW — For dedup key only. Collapses tab variants to canonical form <<<
// "February 16 2026", "2/16/2026", "Cohorts February 16 2026" → same week → same key
function canonicalSourceTabForDedup_(val) {
  ...
  if (str === '') return '';
  var approx = getTabApproxDate_(str);
  if (approx) {
    return (approx.getMonth() + 1) + '/' + approx.getDate() + '/' + approx.getFullYear();
  }
  return str;
}
